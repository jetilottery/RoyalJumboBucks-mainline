define(require => {
  ...
  msgBus.subscribe('PrizeData.PrizeTable', function(){
    ...
    prizeTable.push(SKBeInstant.formatCurrency({amount: prizeData.prizeTable.IW4, numberOfDecimalsToReturn: 0}).formattedAmount);
    prizeTable.push(SKBeInstant.formatCurrency({amount: prizeData.prizeTable.IW3, numberOfDecimalsToReturn: 0}).formattedAmount);
    prizeTable.push(SKBeInstant.formatCurrency({amount: prizeData.prizeTable.IW2, numberOfDecimalsToReturn: 0}).formattedAmount);
    prizeTable.push(SKBeInstant.formatCurrency({amount: prizeData.prizeTable.IW1, numberOfDecimalsToReturn: 0}).formattedAmount);
  ...
  function bonusGameReady(){
    ...
    displayList.autoPlayStartButton.label.text = resources.i18n.Game.button_autoReveal;
  ...
  function enablePickPoints(){    
    ...
    return pickPoints.map(async pickPoint => {
      ...
      //audio.play('Reveal', false, 0.5);
      Tween.delayedCall(0.5, function(){
        audio.play('ChestReveal', false, 0.5);  
      });
      ...
      if (pickPointsClickedArr.length === bonusScenario.length){
        ...
        // Update the button label
        displayList.autoPlayStartButton.label.text = resources.i18n.Game.button_autoPlay;
      }
  ...
  function winPresentation(count, lastPickPoint){
    ...
    if (canPlayParticles){      
            // wait so that the particles waits for the reveal animation on the chest is completed! :)
            Tween.delayedCall(0.5, function(){
  ...
  // RJBUCKS-57 - RJBUCKS_IXFWLA:red background isn't centered in bonus page after rotate screen
  // We need to take account of positioning
  msgBus.subscribe('GameSize.OrientationChange', () => {
    if (currentPrizeTableLevel > 0 && displayList['prizeLevel'+currentPrizeTableLevel]){
      // Add a setTimeout of 0 so that it's the last thing to be updated
      setTimeout(() => {
        displayList.prizeTableAnim.x = displayList['prizeLevel'+currentPrizeTableLevel].x;
        displayList.prizeTableAnim.y = displayList['prizeLevel'+currentPrizeTableLevel].y;
      }, 0);      
    }    
  });
